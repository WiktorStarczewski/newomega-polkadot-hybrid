...
pub use self::newomegatokens::NewOmegaTokens;
...
mod newomegatokens {
    ...
    pub struct NewOmegaTokens {
        ...
        creator: AccountId,
        /// Authorised operators of this contract
    ...
    impl NewOmegaTokens {
        ...
        #[ink(constructor)]
        pub fn new() -> Self {
            let mut owners: StorageVec<AccountId> = StorageVec::default();
            let caller = Self::env().caller();
            owners.push(caller);

            Self {
                balances: StorageHashMap::default(),
                approvals: BTreeMap::default(),
                token_id_nonce: TokenId::default(),
                creator: caller,
                owners,
            }
        }
        ...
        #[ink(message)]
        pub fn authorise_storage_contract(&mut self, storage_address: AccountId) {
            assert!(self.creator == self.env().caller());
            self.owners.push(storage_address);
        }
        ...
        pub fn create(&mut self, value: Balance) -> TokenId {
            ...
            if self.owners.iter().len() > 0 {
                assert!(self.owners.iter().any(|owner| *owner == caller));
            }
        ...
        #[ink(message)]
        pub fn create_for(&mut self, value: Balance, for_account: AccountId) -> TokenId {
            let caller = self.env().caller();
            if self.owners.iter().len() > 0 {
                assert!(self.owners.iter().any(|owner| *owner == caller));
            }
            ...
            // Given that TokenId is a `u128` the likelihood of this overflowing is pretty slim.
            self.token_id_nonce += 1;
            self.balances.insert((for_account, self.token_id_nonce), value);

            // Emit transfer event but with mint semantics
            self.env().emit_event(TransferSingle {
                operator: Some(caller),
                from: None,
                to: if value == 0 { None } else { Some(for_account) },
                token_id: self.token_id_nonce,
                value,
            });
        ...
        #[ink(message)]
        pub fn burn_for(&mut self, token_id: TokenId, value: Balance, for_account: AccountId) -> Result<()> {
            ensure!(token_id <= self.token_id_nonce, Error::UnexistentToken);
            ...
            self.balances
                .entry((for_account, token_id))
                .and_modify(|b| *b -= value);

            // Emit transfer event but with burn semantics
            // TODO verify the burn semantics ;)
            self.env().emit_event(TransferSingle {
                operator: Some(caller),
                from: Some(for_account),
                to: None,
                token_id,
                value,
            });
        ...
        #[ink(message)]
        pub fn mint_for(&mut self, token_id: TokenId, value: Balance, for_account: AccountId) -> Result<()> {
            ensure!(token_id <= self.token_id_nonce, Error::UnexistentToken);
            ...
            if self.balances.get(&(for_account, token_id)).is_none() {
                self.balances.insert((for_account, token_id), value);
            } else {
                self.balances
                    .entry((for_account, token_id))
                    .and_modify(|b| *b += value);
        ...
        // IMPROVEME This should be handled off chain via a RPC and events directly
        #[ink(message)]
        pub fn get_all_tokens_for(&self, for_account: AccountId) -> Vec<TokenId> {
            self.balances
                .iter()
                .filter_map(|entry| {
                    let (&key, &value) = entry;
                    let (account_id, token_id) = key;
                    if (account_id != for_account) || (value == 0) {
                        None
                    } else {
                        Some(token_id)
                    }
    ...
    impl super::Erc1155 for NewOmegaTokens {
        #[ink(message)]
        fn safe_transfer_from(
            &mut self,
            from: AccountId,
        ...
        fn is_approved_for_all(&self, owner: AccountId, operator: AccountId) -> bool {
            if self.owners.iter().len() > 0 {
                if self.owners.iter().any(|self_owner| *self_owner == operator) {
                    return true;
                }
            }
    ...
    impl super::Erc1155TokenReceiver for NewOmegaTokens {
        ...
        fn on_received(
            ...
            ON_ERC_1155_RECEIVED_SELECTOR[..].to_vec()
        }
        ...
        fn on_batch_received(
            ...
            // This prevents a user from unintentionally transferring tokens to a smart contract
            // and getting their funds stuck without any sort of recovery mechanism.
            _ON_ERC_1155_BATCH_RECEIVED_SELECTOR.to_vec()
        }
    }
    ...
    mod tests {
        ...
        fn init_contract() -> NewOmegaTokens {
            let mut erc = NewOmegaTokens::new();
        ...
        fn can_get_correct_balance_of() {
            ...
            let tokens = erc.get_all_tokens_for(alice());
